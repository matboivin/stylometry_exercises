...
from textools.textools.removal import remove_punctuation
from textools.textools.removal import remove_stopwords
...
def combine_files(indices: List[int]) -> str:
    ...
    for index in indices:
        with open(f"{DATA_DIR}/federalist_{index}.txt") as f:
            files.append(f.read())
...
def tokenize_corpus(corpus: Dict[str, str]) -> Dict[str, List[str]]:
    ...
    for category in CATEGORIES:
        tokens: List[str] = nltk.word_tokenize(remove_punctuation(
            corpus[category]),
